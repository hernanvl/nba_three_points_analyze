...
Original file is located at
    https://colab.research.google.com/drive/1J0wmf4cPXq-UN71fMXSQZuzySUmQOybu

##CLEANED DATA REGULAR SEASON
"""
...
df = pd.read_csv(r"/content/drive/MyDrive/Maestria/MASTER DATA SCIENCE/7. Precourse Python/NBA/Data Lake/NBA_GameLogs_1985-2025regs.csv") #Read the document saved in our Data Lake
...
# Save modified CSV
dfcolumns.to_csv("dfcolumns.csv", index=False)
...
NaNWL = df_selected[df_selected["WL"].isnull()]
print(NaNWL)

"""These are cancelled games, so we proceded to eliminate"""

df_selected.dropna(subset=["WL"],axis=0, inplace=True)

print(df_selected.isnull().sum())

"""There is a problem with the data beacause we have thre point field made but not attempted. We decide to eliminate."""

total_made_no_attempts = df_selected.loc[df_selected['three_point_field_goals_attempted'].isnull(), 'three_point_field_goals_made'].sum()
print(total_made_no_attempts)
...
# filter only the matchs with less than 60 points
...
#create a value where give me the diference where there point field goal made are higger than cero with there point fiels attempted
...
df_selected.to_csv("reg85_25.csv", index=False)